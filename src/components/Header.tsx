...
const Header = () => {
  ...
  return (
    <div className="header">
        ...
        ? (
          <Box display="flex" justifyContent="flex-end" sx={flexContainerStyles} pr={10}>
          <Button onClick={handleClick} size="small" sx={{ ml: 2 }}>
            <Typography mt={3} mb={3} variant="body2" alignSelf="flex-end" color={scss_variables.primary_color}>
              {`${user.firstName} ${user.lastName}`}
            </Typography>
            <Avatar src={user.profileImage || 'https://st3.depositphotos.com/1767687/16607/v/450/depositphotos_166074422-stock-illustration-default-avatar-profile-icon-grey.jpg'} />
          </Button>
              <Menu
                anchorEl={anchorEl}
                open={open}
        ...
        :(
          <Box display="flex" justifyContent="flex-end" sx={flexContainerStyles} pr={10}>
            <Button color="inherit" component={Link} to="/login" sx={{ mr: 2, color:'#003066' }}>
              Login
            </Button>
            <Button color="inherit" component={Link} to="/register" sx={{color:'#003066'}}>
              Register
            </Button>
          </Box>
        )}
