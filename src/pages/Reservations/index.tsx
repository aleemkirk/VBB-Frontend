...
const Reservations = () => {
    ...
    const reservationsState = useSelector((store: AppState) => store.library.reservations);
    const userPrefSlots = useSelector((store: AppState) => store.library.user_preference_slots);

    const [activePaginationIndex, set_activePaginationIndex] = React.useState(1);
    const [activePaginationIndex2, set_activePaginationIndex2] = React.useState(1);

    const [numOfItems, set_numOfItems] = React.useState(10);
    const [reservations, set_reservations] = React.useState<any>(null);
    const [user_preference_slots, set_user_preference_slots] = React.useState<any>(null);


    const [toggleView, set_toggleView] = React.useState(false);


    const [addUserPrefSlotModalOpen, set_addUserPrefSlotModalOpen] = React.useState(false);
    const [editUserPrefSlotModalOpen, set_editUserPrefSlotModalOpen] = React.useState(false);
    const [deleteUserPrefSlotConfirmModalOpen, set_deleteUserPrefSlotConfirmModalOpen] = React.useState(false);
    const [activeUserPrefSlotToDelete, set_activeUserPrefSlotToDelete] = React.useState<any>(null);
    const [activeUserPrefSlotForm, set_activeUserPrefSlotForm] = React.useState<any>(defaultForm);
    const [activeUserPrefSlot, set_activeUserPrefSlot] = React.useState<any>(null);


    const [addReservationModalOpen, set_addReservationModalOpen] = React.useState(false);
    const [editReservationModalOpen, set_editReservationModalOpen] = React.useState(false);
    const [deleteReservationConfirmModalOpen, set_deleteReservationConfirmModalOpen] = React.useState(false);
    const [activeReservationToDelete, set_activeReservationToDelete] = React.useState<any>(null);
    const [activeReservationForm, set_activeReservationForm] = React.useState<any>(defaultForm);
    ...
    React.useEffect(() => {
      if (reservationsState !== undefined && reservationsState !== null) {
        ...
        let newSort = reservationSort.sort(function(a:any, b:any){return new Date(a.createdAt).valueOf() - new Date(b.createdAt).valueOf()});
    ...
    React.useEffect(() => {
      if (userPrefSlots !== undefined && userPrefSlots !== null) {
        ...
        let newSort = prefSlotSort.sort(function(a:any, b:any){return new Date(a.createdAt).valueOf() - new Date(b.createdAt).valueOf()});
    ...
    const ReservationRow = ({ reservation }: {reservation:any}) => {
      ...
      return(
        ...
        >
          ...
          <TableCell>
            <Box display="flex" alignItems="center" flexWrap="wrap">
              <PersonIcon /> {reservation.student && reservation.student.firstName || '-'}  {reservation.student && reservation.student.lastName || '-'}
            </Box>
          </TableCell>
          <TableCell><PersonIcon /> {reservation.mentor && reservation.mentor.firstName || '-'} {reservation.mentor && reservation.mentor.lastName || '-'}</TableCell>
          <TableCell>{moment(reservation.createdAt).format('MM/DD/YY HH:MM A') || '-'}</TableCell>
          <TableCell align="right">
            ...
            <Menu
               ...
               >
                 <MenuItem onClick={()=>handleToggleActiveReservationView(reservation)}>View/Edit...</MenuItem>
                 <MenuItem onClick={()=>handleToggleDeleteReservationConfirm(reservation)}><div style={{color:"red"}}>Remove...</div></MenuItem>
             </Menu>
          </TableCell>
    ...
    const renderReservations = (reservations: any[], filterTxt:string) => {
      ...
      if (filterTxt === "") {
        ...
        filteredList = reservations.filter((item:any) => item.name && item.name.toLowerCase().includes(filterTxt.toLowerCase()));
      }
    ...
    */
    ...
    const UserPreferenceSlotRow = ({ userPrefSlot }: {userPrefSlot:any}) => {
      ...
      return(
        ...
        >
          ...
          <TableCell scope="column">
            <b>{userPrefSlot.isRecurring ? 'Yes' : 'No'}</b><br/>
            {userPrefSlot.isRecurring ? `End Date: ${moment(userPrefSlot.endRecurring).format('MM/DD/YYYY')}` : null}

          </TableCell>
          <TableCell>
            <Box display="flex" alignItems="center" flexWrap="wrap">
              <PersonIcon /> {userPrefSlot.student && userPrefSlot.student.firstName || '-'}  {userPrefSlot.student && userPrefSlot.student.lastName || '-'}
            </Box>
          </TableCell>
          <TableCell><PersonIcon /> {userPrefSlot.mentor && userPrefSlot.mentor.firstName || '-'} {userPrefSlot.mentor && userPrefSlot.mentor.lastName || '-'}</TableCell>
          <TableCell>{moment(userPrefSlot.createdAt).format('MM/DD/YY HH:MM A') || '-'}</TableCell>
          <TableCell align="right">
            <Button
               id="positioned-button"
               aria-controls={open ? 'positioned-menu' : undefined}
            ...
            <Menu
               ...
               >
                 <MenuItem onClick={()=>handleToggleActiveUserPrefView(userPrefSlot)}>View/Edit...</MenuItem>
                 <MenuItem onClick={()=>handleToggleDeleteUserPrefConfirm(userPrefSlot)}><div style={{color:"red"}}>Delete...</div></MenuItem>
             </Menu>
          </TableCell>
    ...
    const renderUserPrefSlots = (userPrefSlots: any[], filterTxt:string) => {
      ...
      if (filterTxt === "") {
        ...
        filteredList = userPrefSlots.filter((item:any) => item.firstName && item.firstName.toLowerCase().includes(filterTxt.toLowerCase()));
      }
      renderList = filteredList.map(userPrefSlot => {
        return(
          <UserPreferenceSlotRow userPrefSlot={userPrefSlot}/>
        )
      });
      return renderList
    };
    ...
    function handleEditEndDate(e:any) {
      ...
      if (newEndDate) {
        set_activeReservationForm({ ...activeReservationForm, displayEnd: newEndDate.toISOString()})
      }
    }
    ...
    return(<>
      <BasicModal open={deleteReservationConfirmModalOpen} onClose={()=>set_deleteReservationConfirmModalOpen(false)} title={'Delete Reservation?'}>
        ...
        <Typography mt={1} mb={1} variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
          ...
          <Button onClick={()=>handleToggleDeleteReservationConfirm(activeReservationToDelete)} variant="contained" color="error" sx={{mt:2}} >
            No, cancel.
          </Button>
        </Box>
      </BasicModal>

      <BasicModal open={deleteUserPrefSlotConfirmModalOpen} onClose={()=>set_deleteUserPrefSlotConfirmModalOpen(false)} title={'Delete User Preference Slot?'}>
        <Box display={'flex'} flexWrap={'wrap'} width={"100%"} flexDirection={'column'} justifyContent={'flex-start'}>
        <Typography mt={1} mb={1} variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
          Are you sure you want to delete this user preference slot? It will remove all connected computer reservations for this student.
        </Typography>
        ...
        <Typography mt={1} mb={1} variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
          Recurring Start - End Date & Time: <b>{activeUserPrefSlotToDelete ? `${activeUserPrefSlotToDelete.startRecurring && moment(activeUserPrefSlotToDelete.startRecurring).format('dddd, MMMM Do hh:mm a') || '-'} - ${activeUserPrefSlotToDelete.endRecurring && moment(activeUserPrefSlotToDelete.endRecurring).format('dddd, MMMM Do') || '-'}`:`No user preference time...`}</b>
        </Typography>
      ...
      <BasicModal open={editReservationModalOpen} onClose={()=>set_editReservationModalOpen(false)} title={'Edit Reservation'}>
        <Box mt={2} display={'flex'} flexWrap={'wrap'} width={"100%"} flexDirection={'column'} justifyContent={'flex-start'}>
          ...
          <Box display="flex" flexDirection="row" justifyContent="flex-end">
            ...
            <Button type="submit" onClick={()=>handleEditReservation(activeReservationForm)} variant="contained" color="info" sx={{mt:2, ml:2}} >
              {appState.loading
              ? (<CircularProgress />)
              : (`Save`)
              }
      ...
      <BasicModal open={editUserPrefSlotModalOpen} onClose={()=>set_editUserPrefSlotModalOpen(false)} title={'Edit Student Preference Slot'}>
        <Box mt={2} display={'flex'} flexWrap={'wrap'} width={"100%"} flexDirection={'column'} justifyContent={'flex-start'}>
          <>
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              <FormLabel>Student</FormLabel>
              <b>{activeUserPrefSlot && activeUserPrefSlot.student?.firstName} {activeUserPrefSlot && activeUserPrefSlot.student?.lastName}</b>
            </FormControl>
            <br/>
            <br/>
            <FormControl fullWidth>
              ...
              {activeUserPrefSlotForm !== undefined && activeUserPrefSlotForm !== null
                ? (
                  <MentorUserDropdown
                    ...
                    isRequired={false}
                    defaultValue={activeUserPrefSlotForm.mentor ? activeUserPrefSlotForm.mentor?.pk : null}
                    handleSelectUser={(id) =>
                      set_activeUserPrefSlotForm({ ...activeUserPrefSlotForm, mentor: id })
                    }
                  />
                )
                : null
            ...
            <FormControl fullWidth>
              ...
              <LocalizationProvider dateAdapter={AdapterMoment}>
               <DateTimePicker
                 ...
                 onChange={(newValue) => {
                   set_activeUserPrefSlotForm({ ...activeUserPrefSlotForm, startTime:newValue })
                 }}
               />
             </LocalizationProvider>
            </FormControl>
              ...
              <FormLabel>End Date & Time</FormLabel>
               <LocalizationProvider dateAdapter={AdapterMoment}>
                <DateTimePicker
                  ...
                  onChange={(newValue) => {
                    set_activeUserPrefSlotForm({ ...activeUserPrefSlotForm, endTime:newValue })
                  }}
                />
              </LocalizationProvider>
            </FormControl>
              ...
              <FormLabel>Start Recurring Date & Time</FormLabel>
               <LocalizationProvider dateAdapter={AdapterMoment}>
                <DateTimePicker
                  ...
                  disabled={true}
                  onChange={(newValue) => {
                    set_activeUserPrefSlotForm({ ...activeUserPrefSlotForm, startRecurring:newValue })
                  }}
                />
              </LocalizationProvider>
            </FormControl>
              ...
              <FormLabel>End Recurring Date & Time</FormLabel>
               <LocalizationProvider dateAdapter={AdapterMoment}>
                <DateTimePicker
                  ...
                  onChange={(newValue) => {
                    set_activeUserPrefSlotForm({ ...activeUserPrefSlotForm, endRecurring:newValue })
                  }}
                />
              </LocalizationProvider>
            </FormControl>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <br/>
              <Typography variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
                Recurring?:<b>{activeUserPrefSlotForm.isRecurring ? 'Yes' : 'No'}</b><br/>
                <b>{activeUserPrefSlotForm.isRecurring ? `End Date: ${moment(activeUserPrefSlotForm.endRecurring).format('MM/DD/YYYY')}` : null}</b>

              </Typography>
              <br/>
              <br/>
              <Typography variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
                Created At: <br/><b>{activeUserPrefSlotForm && moment(activeUserPrefSlotForm.createdAt).format('MM/DD/YY HH:MM A')}</b>
              </Typography>
              <br/>
              <br/>
              <Typography variant="body1" alignSelf="flex-start" color={scss_variables.primary_color}>
              ...
              <Button onClick={()=>handleToggleDeleteUserPrefConfirm(activeUserPrefSlot)} variant="contained" color="error" sx={{mt:2}} >
                Delete
              </Button>
            </Grid>
          </Grid>
          <Box display="flex" flexDirection="row" justifyContent="flex-end">
            <Button onClick={()=>handleToggleActiveUserPrefView(activeUserPrefSlot)} variant="contained" color="error" sx={{mt:2}} >
              Cancel
            </Button>
            <Button type="submit" onClick={()=>handleEditUserPrefSlot(activeUserPrefSlotForm)} variant="contained" color="info" sx={{mt:2, ml:2}} >
              {appState.loading
              ? (<CircularProgress />)
              : (`Save`)
              }
      ...
      <BasicModal open={addReservationModalOpen} onClose={()=>set_addReservationModalOpen(false)} title={'Add Reservation'}>
        <Box mt={2} display={'flex'} flexWrap={'wrap'} width={"100%"} flexDirection={'column'} justifyContent={'flex-start'}>
          <>
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
          ...
          <Box display="flex" flexDirection="row" justifyContent="flex-end">
            ...
            <Button type="submit" onClick={()=>handleAddReservation(activeReservationForm)} variant="contained" color="info" sx={{mt:2, ml:2}} >
              {appState.loading
              ? (<CircularProgress />)
              : (`Save`)
              }
      ...
      <BasicModal open={addUserPrefSlotModalOpen} onClose={()=>set_addUserPrefSlotModalOpen(false)} title={'Add Student Preference Slot'}>
        <Box mt={2} display={'flex'} flexWrap={'wrap'} width={"100%"} flexDirection={'column'} justifyContent={'flex-start'}>
          <>
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
              ...
              <div className="card-container">
                ...
                ? (
                  <div className="card-body">
                    ...
                    <Box display="flex" flexWrap="wrap" flexDirection="row" alignItems="center" justifyContent="space-between" flex={1} width="100%">
                      ...
                      <Button variant="contained" color="info" onClick={()=>set_toggleView(!toggleView)} sx={{mt:2, ml:2}} >
                        {appState.loading
                        ? (<CircularProgress />)
                           ...
                           {user_preference_slots
                             ? (
                               <>
                                 ...
                                 ? (
                                   <>
                                     {renderUserPrefSlots(paginate(user_preference_slots, activePaginationIndex2, numOfItems).data, filterText2)}
                                   </>
                                 )
                                 : (
                                   <TableRow
                                     ...
                                     sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                                   >
                                     <TableCell component="th" scope="row">No student preference slots...</TableCell>
                                     <TableCell>-</TableCell>
                                     <TableCell>-</TableCell>
                                     <TableCell>-</TableCell>
                                     <TableCell>-</TableCell>
                    ...
                    {user_preference_slots &&
                      <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center">
                        <Pagination shape="rounded" count={paginate(user_preference_slots, activePaginationIndex2, numOfItems).totalPages} page={activePaginationIndex2} onChange={handlePageChange} />
                      </Box>
                    }
                ...
                : (
                  <div className="card-body">
                           ...
                           {reservations
                             ? (
                               <>
                                 ...
                                 ? (
                                   <>
                                     {renderReservations(paginate(reservations, activePaginationIndex, numOfItems).data, filterText)}
                                   </>
                                 )
                                 : (
                                   <TableRow
                                     ...
                                     sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                                   >
                                     <TableCell component="th" scope="row">No reservations...</TableCell>
                                     <TableCell>-</TableCell>
                                     <TableCell>-</TableCell>
                                     <TableCell>-</TableCell>
                                   </TableRow>
                    ...
                    {reservations &&
                      <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center">
                        <Pagination shape="rounded" count={paginate(reservations, activePaginationIndex, numOfItems).totalPages} page={activePaginationIndex} onChange={handlePageChange} />
                      </Box>
                    }
